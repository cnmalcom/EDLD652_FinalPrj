...
#proficiency level for english language learner
 r2a %>% 
  ...
  scale_x_continuous(expand = c(0,0), 
                     limits = c(0,101))+ 
  labs(title = "Percentage of students at and above RLA proficiency across states",
       subtitle = "English Language Learners vs All Students",
         y = "",
       x= "Percent of students at and above RLA proficiency") +
    theme_minimal() +
   theme(plot.title.position = "plot",  #left aligned title and subtitle
...
r2a %>% 
  ...
  scale_x_continuous(expand = c(0,0), 
                     limits = c(0,101))+
    labs(title = "Percentage of students at and above RLA proficiency across states",
       subtitle = "Students with Disabililty vs All Students",
         y = "",
       x= "Percent of students at and above RLA proficiency") +
      theme_minimal() +
   theme(plot.title.position = "plot",  #left aligned title and subtitle
...
r2a %>% 
 ...
 geom_col(aes(ecdrla, stnam), fill = "#F1A8A1")+
  ...
  labs(title = "Percentage of High School students at and above RLA proficiency across states",
       subtitle = "Low-Income Students vs All Students",
         y = "",
       x= "Percent of students at and above RLA proficiency") + 
  theme_minimal() +
   theme(plot.title.position = "plot",  #left aligned title and subtitle
...
#### Relationship between Textbook Spending & RLA / Math Achievement in the State Level
...
ggplot(aes(allrla, fct_reorder(stnam, allrla))) +
  ...
  theme_minimal() #+
  # theme(plot.background = element_rect(fill = "#EAF1DC"))


state_r2b %>% 
  mutate(stnam = fct_reorder(stnam, allrla)) %>% 
ggplot(aes(allrla, fct_reorder(stnam, allrla))) +
  geom_segment(aes(y = stnam, yend = stnam, x = 0, xend = allrla), color = "skyblue", size = 2) +
...
biscale <- state_r2b %>% 
  bi_class(allrla, stperstudbook, style = "quantile", dim = 3) %>% 
  left_join(statepop, by=c("stnam"= "full"))


plot_usmap(data = biscale, values = "bi_class", labels = TRUE, label_color = "white")+
...
state_r2b$rla_z <- round((state_r2b$allrla - mean(state_r2b$allrla)), 2)
state_r2b$rla_type <-ifelse(state_r2b$rla_z < 0, "below", "above")


#Diverging plot with geom_bar

state_r2b %>% 
  mutate(stnam = fct_reorder(stnam, rla_z)) %>% 
  ggplot(aes(rla_z, stnam)) +
  geom_bar(stat = "identity", aes(fill =rla_type), width=0.5)+
  scale_fill_manual(name = "Average RLA", 
                    labels = c("Above Average", "Below Average"),
                    values = c("above" = "#00ba38", "below"="#f8766d")) +
  theme_minimal()+
  labs(title= "Diverging Bars",
       subtitle = "Mean-Centered RLA percentage at and above the proficiency level",
        x = "Normalised Percent of RLA Proficiency",
       y = "")


#Diverging plot with Lollipop Chart

p <- state_r2b %>% 
  mutate(stnam = fct_reorder(stnam, rla_z)) %>% 
  ggplot(aes(rla_z, stnam)) +
  geom_point(stat = "identity", color ="#B07FD7", size=15)+
  geom_segment(aes(y= stnam,
                   x = 0,
                   yend= stnam,
                   xend=rla_z), color = "#B07FD7", size = 2) +
  geom_text(aes(label = rla_z), color = "white", size = 4) + 
...
pmath <- state_r2 %>% 
  mutate(stnam = fct_reorder(stnam, math_z)) %>% 
  ggplot(aes(math_z, stnam)) +
  geom_point(stat = "identity", color ="#D7987F", size=15)+
  geom_segment(aes(y= stnam,
                   x = 0,
                   yend= stnam,
                   xend=math_z), color = "#D7987F", size = 2) +
  geom_text(aes(label = math_z), color = "black", size = 5) + 
  labs(title= "Diverging Lollipop Chart",
       subtitle = "Mean centered percent of Math at and above the proficiency",
       x = "",
       y = "") +
...
us <- lea_get() %>% 
    sf::st_as_sf() 
us_map <- left_join(state_r2b, us, by=c("GEOID" = "leaid") %>% 
  ggplot() +
  geom_sf(aes(fill = perstudbook), size = 0.1) +
  scale_fill_continuous_divergingx(palette = "Earth",  labels = scales::dollar) +
  theme(legend.position = "bottom",
...
# **Research Question 3:** What is the relationship, if any, between textbook expenditure by school district and percent of minority students in the school district's population?
...
### Oregon Colorblind Friendly Version - Visualizing textbook spending for school district

![Color-blind OR Plot](/Users/cassiemalcom/Desktop/CM2020/22_Winter Term Classes/EDLD 652_DS2/EDLD652_FinalPrj/Data/ORtxtbk_colorblind.png)
