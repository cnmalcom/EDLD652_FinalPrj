...
rm(dm) #freeing space  # did not know this, neat!
...
dm_total <- dm_total %>% 
  distinct()  # learning more from you all. Saving these codes in my R notes :)
...
temp <- dm_total %>%
  filter(n ==2) %>% 
  slice(which.max(STUDENT_COUNT)) # I forget what slice() is but have seen another group use it too so need to look it up
...
dm18_long %>% 
    ...
    ggplot(aes(x = race_ethnicity, y = stu_total))+
    geom_col()+
    scale_y_log10()+  # this one code piece makes such a difference!
    coord_flip()+
    facet_wrap(~st)
...
# This is neat but I feel there is a lot going on here. Maybe once you add a title and modify axis titles things will visually look more congruent. Still so many squares... Also, add some color and make it pop! :)
```
...
## this is so cool! Here's the color I was looking for!! The words are kind of squishing together though.
```
...
# Beyond great! I really like the gradient colors chosen.
```
...
# Oh this is a nice comparison- I like this. Great contrasting colors too. I think centering the legend would add to the aesthetics.
```
...
# for some reason the map for White students is cut off by the legend. I moved the legend to the bottom to fix that. But you can also move it to the top of the maps or elsewhere.
```
...
#rla_sc <- import(here("Data", "rla_sc.csv")) file is too big, I can't push it to github  ## did it not work as a parquet file either?
...
# one thing I learned is that I learned from the HW is that you can use readr::parse_number() for as.numeric and you will know what NAs are being introduced by coercion
...
```{r plots}
#Planning to have interactive plots with all student as gray background and other characteristics with color on top of the gray bar. I like this idea, think it would look really good!
#all students 
all %>% 
  filter(identity == "all") %>% 
  group_by(stnam) %>% 
...
# these are nice! I think factoring in descending order like the first will make these other three plots look better.
```
...
cek  
# I'm confused why there is a true/false color legend? Is that true/false from being less than the national average? If so, I think that is repetitive information not needed.
```
...
# Can't run this code, can't find the function 'lea_get'.
```
...
# same here. I'm sure this plot will look great based on previous plots the group has!
```
...
# I have no idea what package I can get to install lea_get function. I looked online for awhile but kept getting websites for leagues. The help document was not existent when I tried that. Where did you all find this function? Sorry I couldn't look through the last plots but things have been looking amazing.
