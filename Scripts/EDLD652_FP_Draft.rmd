...
dm <- read_parquet(here("data","district-membership-17-21.parquet")) #added "data", wasn't loading for me without it
...
#Nice use of tidyverse code
#This is very cool and would let me work on a state level, but would need to be pretty big. Have you considered doing State as the Y axis and doing race as the facet? Code below. Depends on what you're trying to convey, I guess

dm18_long %>% 
  group_by(st, race_ethnicity) %>% 
    summarise(
        percent_d2 = mean(percent_d, na.rm = TRUE)
    )%>% 
    ggplot(aes(x = st, y = percent_d2))+
    geom_col()+
    coord_flip()+
...
#Log scale is a cool idea though! These graphs are a bit small for log scale to be easily interpeted. I don't have much of a suggestion, sorry
```
...
#Yep, looks great! I added code to make text bigger
...
#Super neat! I took out coord_flip. Personally, I'd just remove the text and relegate it to a legend. This is a very cool idea. If states were more uniformly sized, it'd be cool to put these charts into the states themselves. But... they're not
...
#computers are like that. I used clean_names() above for sm_2018, so the code below will run. That'll surely ruin other visualizations though :(


race <- c("percent_s_american_indian_or_alaska_native", "percent_s_asian", "percent_s_black_or_african_american","percent_s_hispanic_latino", "percent_s_native_hawaiian_or_other_pacific_islander", "percent_s_two_or_more_races", "percent_s_white")
...
#These visualizations are great! A bigger title and legend would be helpful, but that's about it for me
```
...
#Nice use of facet_wrap, but I think these would be more legible as individual visualizations
```
...
ggplot(data = subset(stueth_map, race_ethnicity != "White"),
  ...
  scale_fill_continuous_sequential(palette = "Reds 3") +
    ...
    theme(legend.position = "bottom",
          strip.background = element_blank()) 
#Clean code! Makes it much easier for me to ruin your visualization.
#You guys are really on it with these types of geographic plots
#I prefer the other fill palette, although the plasma viridis option is nice too
```
...
#Nice, 2 plots is manageable for me. Magma works for me on this one for some reason
#I changed the font to be a bit more legible. I would do them individually, but I'm lazy
```
...
#Added the same code chunk. These are nice
```
...
#Loading these data isn't working for me, sorry!
```
