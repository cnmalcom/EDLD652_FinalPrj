...
dm18_long %>% 
    ...
    ggplot(aes(x = fct_reorder(race_ethnicity, percent_s), 
               y = percent_s, 
               fill = race_ethnicity))+
...
#RG wow I really liked how this turned out! I added reorder so that the race/ethnicity would be ordered by percent
```
...
# RG I liked the use of color for these but I am wondering if there is a way to facet them together so that you can compare the differences by subgroup
...
#These are very informative, could you annotate certain states with $ amounts for added context?
```
...
cek <- all %>% 
  ...
  labs(title = "Relationship between Textbook Spending per Student and Reading proficiency", 
       x= "Percent of students at and above proficiency level",
       y="",
       size= "Spending on textbook per student (dollars)",
       color="",
       legend= "") +
   theme(legend.position = "bottom",
...
#RG I added text to show that the size of the circles represent dollars.

#RG The size of the circles don't vary much. It might make more sense to have per student book as color instead of differentiating low vs. high
```
...
#RG Might be interesting to compare what books cost in each state to see if the cost is inflated due to cost.
```
...
#RG could you annotate cities with higher textbook funding vs. lower?
```
