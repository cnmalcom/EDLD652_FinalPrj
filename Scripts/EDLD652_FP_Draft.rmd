...
dm18_long %>% 
    ...
    mutate(
        total = sum(stu_total),
        percent_s = round((stu_total*100/total),3) #this is really smart!
    ) %>% 
...
dm18_long %>% 
    ...
    ggplot(aes(x = race_ethnicity, 
               y = percent_s, 
               fill = race_ethnicity))+ #Possibly a different color palette would improve readability — the rainbow palette implies some order that your data don't imply. 
    geom_col(show.legend = FALSE)+
    coord_flip()+
    theme_bw() +
    facet_geo(~ st, grid = "us_state_grid2") #This is so fun! I'm not sure it's the easiest thing to interpret though
...
#This is a very effective pair of plots for comparison. Adding titles and changing the legend label would help make them more interpretable (especially making the titles match for both maps)
```
...
#This is an interesting approach...I think logarithmic scales might be more effective for plots that have axis labels, as I worry that this is somewhat misleading because the color gradient feels like it's going to be consistent. Not sure though, just the impression I get from it - it is definitely more effective for communicating nuance in the maps where there is very little density in the corresponding racial/ethnic group. Possibly including both would be useful but making sure to label them separately — or even including a section where you only include this for the groups where differences by state are hard to see above?
```
...
va <- lea_get(state = "va") %>% #I'm not sure what this does, I get an error saying there's no "lea_get" function — is this in some package that isn't loaded here?
    sf::st_as_sf() 
